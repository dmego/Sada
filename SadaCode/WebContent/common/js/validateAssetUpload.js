...
(function ($) {
    ...
    $(function () {
        ...
        uploader = WebUploader.create({
            ...
            chunked: false,
            chunkSize: 512 * 1024,
            server: 'validateFileUploadServlet',
    ...
    var content = '<div class="wrapper wrapper-content" style="background-color: #ecf0f5;padding: 20px;">\
        <div class="row">\
            <div class="col-md-6 col-xs-12">\
                <div class="widget widget-article widget-shadow">\
                    <div class="widget-header cover overlay overlay-hover">\
                        <img class="cover-image overlay-scale" src="' + basePath + filePath + '/' + fileName + '" alt="">\
                    </div>\
                    <div class="widget-body">\
                        <div class="ibox float-e-margins">\
                            <div>\
                                <h4 class="widget-title">我的身份证</h4>\
                            </div>\
            ...
            <div class="col-md-6 col-xs-12">\
                <div class="widget widget-article widget-shadow">\
                    <div class="widget-header cover overlay overlay-hover">\
                        <img class="cover-image overlay-scale" src="' + basePath + chainAsset.filePath + '/' + chainAsset.fileName + '" alt="">\
                    </div>\
                    <div class="widget-body">\
                        <div class="ibox float-e-margins">\
                            <div>\
...
//                                alert(chainKeyInfo);
    ...
    if(diffResult != ''){
        ...
        $("#validateResult").addClass("alert alert-warning alert-dismissible");
        $("#validateResult").html(diffResult + "数据不一致");
    }
    else{
        $("#validateResult").removeClass();
        $("#validateResult").addClass("alert alert-success");
        $("#validateResult").html("鉴权成功，数据一致！");
    }
...
//    alert("进来了");
    ...
    $('body').loading({
        ...
        timeSpan: [0.5,0.5,1,0.3,0.3,0.3,0.3,1,1], //每条描述显示的时间
        direction: 'column',
        type: 'origin',
        //originBg:'#71EA71',
        originDivWidth: 40,
...
//---------------------------
                    ...
                    if(diffResult != ''){
                        ...
                        $("#validateResult").html("<p style='color:#f39c12;text-align:center;'>" + diffResult + "与原件信息不一致,鉴权资产被篡改！</p>");
                    }
                    else{
                        $("#validateResult").removeClass();
...
function toCompareMd5() {
    modals.hideWin("coreAuthWin");
    $('body').addClass('modal-open');
    // var id = $("#id").val();
    localStorage.setItem("compareData", compareData);
    localStorage.setItem("commonType", commonType);
    window.loadPage(basePath + "/assetAction_toCompareMd5.do");
}
