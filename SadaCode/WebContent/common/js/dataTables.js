...
//'use strict';
    ...
    CommonTable.prototype.initTable = function (tableId, queryId, searchDiv) {
        ...
        for (var i = 0; i < this.data.query.columnList.length; i++) {
            ...
            var obj = {};
            obj["data"] = column.key; //"deptId"
            obj["title"] = column.header; //"所在部门"
            obj["name"] = column.key; //"deptId"
            obj["visible"] = !column.hidden; //是否显示:
            obj["sortable"] = column.allowSort; //是否允许排序
            obj["class"] = "text-" + column.align; //样式
        ...
        this.table = $('#' + tableId).DataTable($.extend({
            ...
            "bAutoWidth": true,
            "bStateSave" : true,// 这样就可以在删除返回时，保留在同一页上
            ...
            "serverSide": true,
            "ajaxDataProp": "data",
            "ajaxSource": basePath + "/userAction_loadData.do",
...
//通用选择
    ...
    CommonTable.prototype.getServerData = function (pageInfo, tableId) {
        ...
        ajaxPost(basePath + "/queryAction_loadData.do", {"reqObj": this.toJSONString(reqParam)}, function (result, status) {
            retData = result;
        });
        if (retData == null) return null;
        var start = 0; //开始的显示的行号,
        if (pageInfo) {//如果有分页,按照当前第几页开始显示
            start = pageInfo.pageSize * (pageInfo.pageNum - 1)
        }
        var columns = retData.query.columnList; //获取表单表头行列表信息
            ...
            for (var i = 0; i < retData.rows.length; i++) {
                ...
                if(column.showPic){
                    if(value != null && value != ""){
                        ...
                        filepath=basePath+"common/images/avatar.jpg";
                    }
                    eval(value_str + "=\"<img class='userImage' src='\" + filepath + \"' ></img>\"");
                ...
                if (column.dateFormat) {
                    ...
                    eval(value_str + "=datetimeFormat(value, column.dateFormat)");
                    value = eval(value_str);
                }
                // 格式化数字
