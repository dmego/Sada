	...
	$.fn.loading = function (options) {
		...
		return this.each(function () {
			...
			var defaultProp = {
				...
				//--------------------------------------------------------
				discription: ['这是第一个描述'], //loading的描述
				timeSpan: [1], //每条描述的时间间隔
				//------------------------------------------------------
			...
			defaultProp._showOriginLoading = function () {
				...
				//中间的显示层
				_this.div_loading = $('<div id="div-loading" class="div-loading"></div>').css({
				...
				var interval = setInterval(function () {
					...
					if (i == 0) {
						_this.loading_discription = $('<div id="' + i + '" class="loading-discription txt-textOneRow"></div>').css({
							color: opt.discColor,
						}).html(opt.discription[i]).appendTo(_this.div_loading);
					} else {
