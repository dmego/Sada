...
def idcardocr(imgname):
        ...
        #showimg(sex_pic)
        # print 'sex'
        result_dict['性别'] = get_sex(sex_pic)
        # print result_dict['sex']

        nation_pic = find_nation(img_data_gray, img_org)
        #showimg(nation_pic)
        # print 'nation'
        result_dict['民族'] = get_nation(nation_pic)
        ...
        result_dict['出生'] = "%s年%s月%s日"%(year.encode('utf8'), month.encode('utf8'), day.encode('utf8'))
        ...
        #showimg(address_pic)
        # print 'address'
        result_dict['住址'] = get_address(address_pic)
        # print result_dict['address']

        idnum_pic = find_idnum(img_data_gray, img_org)
        #showimg(idnum_pic)
        # print 'idnum'
        result_dict['公民身份号码'] = get_idnum(idnum_pic)
...
def find_name(crop_gray, crop_org):
        template = cv2.UMat(cv2.imread('%s/idcard/mask/name_mask_%s.jpg' % (currentPath,pixel_x), 0))
        # print '%s/idcard/mask/name_mask_%s.jpg' % (currentPath,pixel_x)
        # showimg(crop_org)
...
def find_sex(crop_gray, crop_org):
        template = cv2.UMat(cv2.imread('%s/idcard/mask/sex_mask_%s.jpg' % (currentPath,pixel_x), 0))
        # showimg(template)
        w, h = cv2.UMat.get(template).shape[::-1]
        res = cv2.matchTemplate(crop_gray, template, cv2.TM_CCOEFF_NORMED)
        min_val, max_val, min_loc, max_loc = cv2.minMaxLoc(res)
...
def find_nation(crop_gray, crop_org):
        template = cv2.UMat(cv2.imread('%s/idcard/mask/nation_mask_%s.jpg' % (currentPath,pixel_x), 0))
        #showimg(template)
        w, h = cv2.UMat.get(template).shape[::-1]
        res = cv2.matchTemplate(crop_gray, template, cv2.TM_CCOEFF_NORMED)
        min_val, max_val, min_loc, max_loc = cv2.minMaxLoc(res)
...
def find_birth(crop_gray, crop_org):
        template = cv2.UMat(cv2.imread('%s/idcard/mask/birth_mask_%s.jpg' % (currentPath,pixel_x), 0))
        # showimg(template)
        w, h = cv2.UMat.get(template).shape[::-1]
        res = cv2.matchTemplate(crop_gray, template, cv2.TM_CCOEFF_NORMED)
        #showimg(crop_gray)
        ...
        # 提取年份
        template = cv2.UMat(cv2.imread('%s/idcard/mask/year_mask_%s.jpg' % (currentPath,pixel_x), 0))
        ...
        # 提取月
        template = cv2.UMat(cv2.imread('%s/idcard/mask/month_mask_%s.jpg' % (currentPath,pixel_x), 0))
        ...
        # 提取日
        template = cv2.UMat(cv2.imread('%s/idcard/mask/day_mask_%s.jpg' % (currentPath,pixel_x), 0))
...
def find_address(crop_gray, crop_org):
        template = cv2.UMat(cv2.imread('%s/idcard/mask/address_mask_%s.jpg' % (currentPath,pixel_x), 0))
        # showimg(template)
        #showimg(crop_gray)
        w, h = cv2.UMat.get(template).shape[::-1]
...
def find_idnum(crop_gray, crop_org):
        template = cv2.UMat(cv2.imread('%s/idcard/mask/idnum_mask_%s.jpg' % (currentPath,pixel_x), 0))
        # showimg(template)
        #showimg(crop_gray)
        w, h = cv2.UMat.get(template).shape[::-1]
        res = cv2.matchTemplate(crop_gray, template, cv2.TM_CCOEFF_NORMED)
